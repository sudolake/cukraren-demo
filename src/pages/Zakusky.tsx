...
export const Zakusky = () => {
  ...
  useEffect(() => {
    ...
    else setActiveFilter('Všetky');
  }, [searchParams]);

  const filteredAndSortedData = useMemo(() => {
    let result = zakuskyData.filter(item => item.type === 'zakusok');
    if (activeFilter === 'Všetky') {
      result = result.filter(item => !item.categories.includes('Banketové'));
    } else {
    ...
    <div className="pt-32 pb-20 min-h-screen bg-brand-pink/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h1 
            className="text-5xl md:text-6xl font-bold mb-6 font-sans text-brand-dark cursor-pointer hover:text-brand-accent transition-colors"
            onClick={() => setActiveFilter('Všetky')}
          >
            Tradícia, ktorú cítite v každom súste.
          </h1>
          ...
          <AnimatePresence>
            {filteredAndSortedData.map((item) => (
              <motion.div
                ...
                key={`${item.name}-${item.categories.join('-')}`}
              ...
              >
                ...
                ) : item.categories.includes('Banketové') ? (
                  <div className="pt-4 text-center">
                  </div>
                ) : (
                  <div className="h-48 bg-brand-pink/10 rounded-2xl mb-6 flex items-center justify-center p-6 text-center">
                    <span className="text-brand-accent/40 font-medium italic">Bez fotky</span>
                  </div>
                )}
                <h3 className={`text-xl font-bold mb-4 font-sans ${(!item.image || item.image === '') && item.categories.includes('Banketové') ? 'text-center' : ''}`}>{item.name}</h3>
                <div className={`mt-auto flex flex-wrap gap-2 ${(!item.image || item.image === '') && item.categories.includes('Banketové') ? 'justify-center' : ''}`}>
