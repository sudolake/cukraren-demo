...
export const SuchePecivo = () => {
  ...
  const filteredAndSortedData = useMemo(() => {
    let result = zakuskyData.filter(item => item.type === 'pecivo');
    // If only one category remains, we might not need the filter UI as much, but let's keep it consistent
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
          <div className="flex flex-wrap gap-2">
            {['Všetky', ...filterCategories.pecivo].map(cat => (
              <button
                key={cat}
        ...
        <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {filteredAndSortedData.length === 0 && (
            <div className="col-span-full py-20 text-center text-gray-500">
              V tejto kategórii momentálne nemáme žiadne pečivo.
            </div>
          )}
        </motion.div>
      </div>
